# Hymmnos Culture and Lore

## Dialects (Notes)

Hymmnos has six dialects, each with different vocabulary, grammar, and power characteristics:

| Dialect | Japanese | Status | Key Features |
|---------|----------|--------|--------------|
| Central Standard Note | 中央正純律 | Living | ISO-like standard by El Elemia; most widely used; EXEC_ songs |
| Kurt Ciel Note | クルトシエール律 | Mostly dead | Pre-First-Era Moon Chanter tongue; basis of Hymmnos |
| Cluster Note | クラスタ律 | Lost near Ar tonelico | First-Era Sol Cluster dialect; near-identical power; Harvestasha server |
| Alpha Note | アルファ律 | Origin-only | Created by 3 Origins; bound to their Tower only |
| Ancient Metafalss Note | 古メタファルス律 | Registered on Ar tonelico | Each word vastly more powerful; spiritual; harder to pronounce |
| New Testament of Pastalie | 新約パスタリエ | Living on Infel Phira | I.P.D. only; METHOD_ songs; incompatible grammar; speed-optimized |

Other forms:
- **Preformalized Lunar Chant** (律史前月読) — earliest form; each letter has a meaning; base of modern forms
- **Ar Ciela** — the planet's own language; predates all Hymmnos; 20-600,000 Hz frequency range

## History Timeline

- A.D. 348: First Hymmnos word "chs" born from Carmena Foreluna
- A.D. 421: First complete sentence "Was yea ra chs hymmnos mea"
- A.D. 1919: Metafalss creates powerful dialect (Ancient Metafalss Note)
- A.D. 2934: Moon Chanters nearly extinct
- A.D. 2973: Sound science researchers discover Hymmnos effects on Sound Stone
- A.D. 3018: El Elemia standardizes 62 characters into current form (Central Standard Note)
- A.D. 3031-3033: Three Origin Reyvateils born; Alpha Note created
- A.D. 3308: New Testament of Pastalie project begins
- A.D. 3400: Pastalie completed for Infel Phira

## Song Types

| Form | Users | Nature |
|------|-------|--------|
| Hymmnos Extract | Reyvateils (beta safe; gamma risky) | Tower control chants; fixed patterns; Hymn Code gated; EXEC_ prefix |
| Hymmnos Words | Reyvateils | From singer's heart; limitless; can work without Tower via Sound Stones |
| Hymmnos Spells | Humans AND Reyvateils | Pre-defined strings; fixed function; tower-registered; no H-waves |

Naming conventions:
- `EXEC_XXXX/.` = Ar tonelico-class servers (Standard + dialects)
- `METHOD_XXXX/.` = Infel Phira (Pastalie only)
- `EXEC_with.METHOD_XXXX/.` = hybrid running both servers
- `EXEC_over.METHOD_XXXX/.` = rear type translated as front type

## Song Magic Servers

| Server | Location | Dialect | Invocation | Power |
|--------|----------|---------|------------|-------|
| Ar tonelico (1st Tower) | Sol Ciel | Central Standard + dialects | EXEC_XXXX/. | Full; Origin Shurelia |
| Sol Marta (2nd Tower relay) | Metafalss | Central Standard (relay) | EXEC_XXXX/. | Via 1st Tower + Frelia |
| Infel Phira | Metafalss | New Testament of Pastalie ONLY | METHOD_XXXX/. | 0.1% of Ar tonelico; IPD-powered |
| Harvestasha (3rd Tower) | Sol Cluster | Central Standard + Cluster | EXEC_XXXX/. | Best-effort; no Orgel |

Servers cannot interoperate natively (different formats, like endianness). Cross-system communication needs a shared protocol.

## Writing System

- Standardized A.D. 3018 by El Elemia Tower Administration Bureau
- 71 characters: 26 uppercase + 26 lowercase + 10 digits + 9 symbols
- Based on "Emission Concentric Circle": center = Sound Source
  - Arching shapes = peace (alpha-waves)
  - Radiating shapes = stimuli (gamma-waves)
  - Vertical/parallel lines = tension (beta-waves)
- Case distinguished by presence/absence of a black dot
- Numbers 0-4 = square waves; pair of dots prefix adds 5
- Free TrueType fonts available from Hymmnoserver

## Hymmnos Script Characters

The 6 main vowel sounds carry fundamental emotional categories:
- A = Power (力音)
- I = Holy (圣音)
- U = Hatred (憎音)
- E = Love (爱音)
- O = Evil (邪音)
- N = Nothingness (无音)

The remaining 20 consonants derive from Carmena Foreluna letter meanings.

## Wave Science

- **H-waves** (Dynamic H-waves): carry the singer's thoughts/feelings; act as program instructions
- **D-waves** (Dynamic D-waves): produce physical effects; act as screen output
- The singer's SHW (high-frequency thoughts) contains the entire song's information at every moment
- Audible waves (50-20,000 Hz D-waves) carry the emotional waves (>20,000 Hz H-waves)
- Each Hymmnos word's pronunciation carries its corresponding emotion at maximum efficiency

## Carmena Foreluna (律史前月読)

The precursor to Hymmnos. Each letter has its own meaning; words are formed by combining letter meanings. Useful for understanding word origins.

| Letter | Meaning | | Letter | Meaning |
|--------|---------|-|--------|---------|
| A | Power | | N | Nothing |
| B | World | | O | Evil |
| C | Change, Growth | | P | Life and Death |
| D | Supernatural, Dark | | Q | Ignorance |
| E | Love | | R | Life |
| F | Convey, Transmit | | S | Wish |
| G | Destruction, Punishment | | T | Me, Oneself |
| H | Flame, Fire | | U | Hatred |
| I | Holy | | V | Joy |
| J | Unfamiliar | | W | Spirit, Soul |
| K | Creation and Destruction | | X | Protection |
| L | You, Other Person | | Y | Light |
| M | Compassion | | Z | God(dess) |

Example: "chs" (become) starts with C = change; "c.z." (Pastalie become) also starts with C.

## Ar Ciela (アル・シエラ)

The planet Ar Ciel's own language, predating all Hymmnos. Each sound carries a feeling; words are sums of letter feelings. Full frequency range: 20-600,000 Hz (30x human hearing).

- Two notation layers: Public (26 symbols, lossy) and Compartment (adds diacritics for inaudible properties)
- FMCL (frequency compartments): `!` `#` `$` `%` for sounds OVER 20,000 Hz (sub-dividing the 20k-600k Hz inaudible range)
- AMCL (amplitude compartments): `&` `(` `)` for waveform shapes
- Vowels have umbrella categories: A=[Scale], I=[Pray], U=[Experience], E=[Neighbour], O=[Karma], N=[Mandala]
- Every letter converges on "love" at the spectrum's far end
- Basis of Carmena Foreluna, which is the basis of Hymmnos

## Key Deities and Names

| Name | Role | Also Known As |
|------|------|---------------|
| Clyuue | King of Gods | Rokkan (六冠) |
| Saki | Goddess of Salvation and Life | Sakiyarumei (咲夜琉命) |
| Horus | Highest God of Earth; manages planetary equilibrium and D-wave phenomena | - |
| Juklizda | God of the Sun | Nayudara (那由他羅) |
| Yurishica | Goddess of Fire and Civilization | Gurentei (紅蓮帝) |
| Shelanoir | Goddess of Plants, Forests and Life | Saranoyomei (沙羅世命) |
| Sasya | Goddess of Knowledge and Civilization | Sashanomiya (紗々ノ宮) |
| Soma | Goddess of Violence and Catastrophe | Soumanomiya (蒼痲ノ宮) |
| Salapator | Goddess of Love and Downfall | Datenkou (堕天后) |
| Filament | Goddess of Death and Poverty | Kokura (黒螺) |
| Hymmnosphere | Goddess of Songs and Creation | Suzunomiya (珠洲ノ宮) |
| Rhaplanca | Goddess from Metafalss mythology | - |
| Maoh | Rhaplanca's partner | Buten (武天) |
